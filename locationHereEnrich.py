...
def procesar_lote_locationHereEnrich() -> str: #esta es la función principal que llama a las otras
    #VARIABLES COMUNES
    ...
    cabecera_location = ["awsHere-IN-id","awsHere-IN-singleLine","CUST-awsHere-orden","awsHere-Label", "awsHere-Latitud", "awsHere-Longitud", "awsHere-AddressNumber", "awsHere-Street", "awsHere-Neighborhood", "awsHere-Municipality", "awsHere-Subregion", "awsHere-Region", "awsHere-PostalCode", "awsHere-Interpolated","awsHere-Categories", "CUST-awsHere-GeoType", "CUSTOM-awsHere-Json", "CUSTOM-awsHere-1erAddType","CUSTOM-awsHere-Estado"]
    ...
    resumenInicio("AWS Location con HERE", max_filas-1, filter_countries, "entrada_locationHere.xlsx")
    ...
    nombre_archivo_generado = generar_archivo_location('OUT_LocatHERE_Enrich - ', cabecera_location, cabecera_address, lista_json, lista_id)
        ...
        try:
            if (len(una_lista_json[json]["Results"]) > 0):
                ...
                for candidato in range (0, len(candidatos_seleccionados[0])):
                    ...
                    una_hoja1_grabando.cell(row=fila_hoja1_escribiendo, column=16).value = candidatos_seleccionados[0][candidato]['tipoGeo'][0]
                    una_hoja1_grabando.cell(row=fila_hoja1_escribiendo, column=19).value = candidatos_seleccionados[0][candidato]['estado']
                    una_hoja1_grabando.cell(row=fila_hoja1_escribiendo, column=17).value = candidatos_seleccionados[0][candidato]['json']
                ...
                una_hoja2_grabando.cell(row=fila_hoja23_escribiendo, column=16).value = candidatos_seleccionados[0][mejor_candidato]['tipoGeo'][0]
                una_hoja2_grabando.cell(row=fila_hoja23_escribiendo, column=19).value = candidatos_seleccionados[0][mejor_candidato]['estado']
                una_hoja2_grabando.cell(row=fila_hoja23_escribiendo, column=17).value = candidatos_seleccionados[0][mejor_candidato]['json']
            ...
            una_hoja1_grabando.cell(row=fila_hoja1_escribiendo, column=19).value = "NE"            
            ...
            una_hoja2_grabando.cell(row=fila_hoja23_escribiendo, column=19).value = "NE"            
...
def trabajar_json(un_json_location, un_json_entrada):
    ...
    for candidate in range (0, len(un_json_location['Results'])):
        ...
        try:        
            tipo_geo = location_here_PE(category, interpolated)
            diccionario_candidato['tipoGeo'] = tipo_geo
        except:
            diccionario_candidato['tipoGeo'] = "s/d"
        
        try:
            estado = estado_openpyxl_basico(tipo_geo)
            diccionario_candidato['estado'] = estado
        except:
            diccionario_candidato['estado'] = "s/d"
        ...
        try:
            ...
            diccionario_candidato['json'] = "s/d"
